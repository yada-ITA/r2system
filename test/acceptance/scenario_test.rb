...
class ScenarioTest < AcceptanceTest
  ...
  test "scenario1 of release0_8" do
    ...
    # 1.4. エンジンを登録する
    select "エンジンモデルコード1", from: "engine_engine_model_name"
    ...
    # 1.5.1. メニューに戻る
    click_link "戻る"
    save_screenshot "scenario1-1_5_1.png"

    # 1.5.2. 整備会社作業一覧を開く
    click_link "整備会社作業一覧"
    save_screenshot "scenario1-1_5_2_1.png"
    assert has_content? "エンジンモデルコード1"
    save_screenshot "scenario1-1_5_2_1.png"

    # 1.6. 受領画面を開く
    within "tbody > tr:nth-of-type(1)" do
      click_link "受領"  # エンジン一覧の1行目にある受領ボタンをクリック
    end
    # 受領時は受領日の登録のみ
    ...
    # 1.7. 整備会社作業一覧画面に移動する
    click_link "戻る"
    save_screenshot "scenario1-1_7_1.png"
    #登録したエンジンが一覧にあることを確認する
    within "tbody > tr:nth-of-type(1)" do
      assert_equal "整備前",  nth_tag(:td, 1).text # ステータス
      assert_equal "エンジンモデルコード1", nth_tag(:td, 7).text # エンジン型式
      assert_equal "SN0001",  nth_tag(:td, 8).text # エンジンNo.
    end
    save_screenshot "scenario1-1_7_2.png"
    # 1.8. サインアウトする
    sign_out
    ...
    select "エンジンモデルコード1", from: "search_engine_model_name"
    ...
    within "tbody > tr:nth-of-type(1)" do
      ...
      assert_equal "こうじめいしょう", nth_tag(:td, 2).text       # 工事名称
    ...
    assert_equal "こうじめいしょう", find_field("工事名称").value
    fill_in "売上金額（見込）", with: "1000000"
    fill_in "sending_postcode", with: "888-9999"
    fill_in "sending_address",  with: "大阪市大淀区大淀中５６７８"
    fill_in "sending_name",     with: "ダミー送付先"
    fill_in "sending_destination_name",   with: "送付咲子"
    fill_in "sending_phone_no", with: "06-333-4444"
    ...
    within "table > tbody > tr:nth-of-type(2)" do
      assert_equal "エンジンモデルコード1 (00000001)", nth_tag(:td, 1).text # 返却エンジン
      assert_equal "エンジンモデルコード1 (SN0001)", nth_tag(:td, 2).text # 新規エンジン
    end
